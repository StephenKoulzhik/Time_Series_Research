...
class Lorentz:
    ...
    def RK4(self, x, y, z, s, r, b, dt):
        ...
        k_3 = self.X((x + k_2 * dt * 0.5),
                     (y + l_2 * dt * 0.5),
                     s)

        l_3 = self.Y((x + k_2 * dt * 0.5),
                     (y + l_2 * dt * 0.5),
                     (z + m_2 * dt * 0.5), r)

        m_3 = self.Z((x + k_2 * dt * 0.5),
                     (y + l_2 * dt * 0.5),
                     (z + m_2 * dt * 0.5), b)
...
def generate_predictions(centers, deamon=None, return_set_pred=False,
                         real_mode='test', EPS=0.05, Q_VALUE=0.99, PTS=100,
                         STEPS=50):
    preds = {}
    set_preds = {}
    ...
    for start_point in tqdm(range(PTS)):
        ...
        for step in range(1, STEPS + 1):
            ...
            last_points = list(map(lambda center: (
                center[1][-1], center[2],
                center[3], center[0]),
                chosen_centers)
            )
